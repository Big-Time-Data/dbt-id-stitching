...
SELECT 'SELECT ' || a.cn || '::TEXT AS edge_a, ''' || a.cn || ''' AS edge_a_label, ' || b.cn || '::TEXT AS edge_b, ''' || b.cn || ''' AS edge_b_label, min({{ var("timestamp-col") }}) as edge_timestamp, min({{ var("event-rank-id-col") }}) as edge_rank_id FROM ' || a.tn || ' WHERE COALESCE(' || a.cn || '::TEXT, '''') != '''' AND COALESCE(' || b.cn || '::TEXT, '''') != ''''' || ' group by ' || a.cn || ', ' || b.cn AS sql_to_run
FROM columns AS a
